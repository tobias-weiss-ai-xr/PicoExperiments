...
# define SOFTMASK_CALCULATE_COORDS(OUT, pos)   (OUT).maskPosition = _SoftMask_Calc_Perspective(OUT.worldPosition);
# define SOFTMASK_GET_MASK(IN)                 SoftMask_GetMask((IN).maskPosition.xy)
	...
	float4 _SoftMask_Plane;

    float4x4 _SoftMask_UIToWorld;
	float4x4 _SoftMask_WorldToLocal;
...
# endif
	...
	float4 _SoftMask_Calc_Perspective(float4 worldPos) {
		worldPos = mul(_SoftMask_UIToWorld, worldPos);
		float3 p0 = -_SoftMask_Plane.xyz * _SoftMask_Plane.w;
		float3 L = worldPos - _WorldSpaceCameraPos;
		float d = dot(p0 - worldPos, _SoftMask_Plane.xyz) / dot(L, _SoftMask_Plane.xyz);
		return mul(_SoftMask_WorldToLocal, float4(d * L + worldPos, 1));
	}
